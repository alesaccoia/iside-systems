...
const schema = { type:"object", additionalProperties:false, required:["title","summary","wins","training","advice"], properties:{ title:{type:"string"}, summary:{type:"string"}, advice:{type:"string"}, wins:{type:"array",minItems:3,maxItems:3,items:{type:"object",additionalProperties:false,required:["title","body"],properties:{title:{type:"string"},body:{type:"string"}}}}, training:{type:"array",minItems:2,maxItems:2,items:{type:"object",additionalProperties:false,required:["title","body"],properties:{title:{type:"string"},body:{type:"string"}}}}}};
export default async function handler(req,res) {
  ...
  const instruction="Sei un advisor AI pragmatico di Iside Systems. Rispondi esclusivamente in italiano corretto, in alfabeto latino, con gli accenti al posto giusto (perch\u00e9, pi\u00f9, gi\u00e0, qualit\u00e0, priorit\u00e0, \u00e8): mai vocali senza accento al posto di quelle accentate. Ti rivolgi all\u0027azienda che ha risposto: non nominare mai Iside Systems come se fosse il cliente. Non promettere risultati certi. Dai priorità a processi reali, dati, adozione e governance. wins contiene esattamente 3 quick win realizzabili nei prossimi 90 giorni; ogni body massimo 230 caratteri. training contiene esattamente 2 voci: la prima è formazione generica di alfabetizzazione AI per tutta l\u0027azienda, la seconda e\u0300 formazione mirata sulla business unit o funzione piu\u0300 sotto pressione secondo le risposte, con esempi del loro settore; ogni body massimo 230 caratteri.";
  try {
